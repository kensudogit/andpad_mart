/**
 * Railway 生存確認用。
 * 統合デプロイでは Java API の起動も確認し、未設定時は 503 を返す。
 */
...
function unifiedApiStatusMessage(): string | undefined {
  const status = process.env.UNIFIED_API_STATUS?.trim()
  if (status === 'missing_database_url') {
  ...
  if (!dbConfigured()) {
    return 'DATABASE_URL is not configured — Java API will not start.'
  }
...
export async function GET() {
  ...
  const message = unifiedApiStatusMessage()
  return Response.json(
    {
      ok: false,
      service: 'andpad-web',
